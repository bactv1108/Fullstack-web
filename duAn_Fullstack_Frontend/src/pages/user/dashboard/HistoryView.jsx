...
export default function HistoryView() {
  ...
  React.useEffect(() => {
    ...
    if (tabParam) {
      if (['all', 'image', 'audio', 'analysis'].includes(tabParam)) {
    ...
    <div className="!w-full !min-h-[calc(100vh-65px)] !bg-[#09090b] text-white !py-8 !px-4 sm:!px-6 lg:!px-8 !block !clear-both">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 border-b border-zinc-900/60 pb-4"
          ...
          <div className="flex items-center gap-1 bg-[#0b0b0e] px-1.5 rounded-xl border border-zinc-900/60 shrink-0 h-[42px] w-full sm:w-auto overflow-x-auto">
            ...
            <button 
              ...
              className={`flex-1 sm:flex-none h-8 px-3.5 text-xs font-black rounded-lg transition-all cursor-pointer flex items-center justify-center ${historyType === 'image' ? 'bg-[#f59e0b] text-black shadow-md shadow-amber-500/10' : 'text-[var(--text-secondary)] hover:text-[var(--text-primary)]'}`}
              style={{padding:'8px'}}>
              Hình ảnh
            </button>
            <button 
      ...
      <div className="!p-6 !bg-[#111114] !border !border-[#222226] !rounded-2xl p-5 sm:p-6 md:p-8 !w-full !shadow-2xl !block">
          ...
          <div className="w-full flex flex-col gap-8 text-left">
            ...
            {/* ------------------------------------------------------- */}
            {/* PHÂN KHU 1: HÌNH ẢNH AI (Chỉ hiển thị ở Tab Tất cả hoặc Tab Hình ảnh) */}
            {/* ------------------------------------------------------- */}
            {(historyType === 'all' || historyType === 'image') && images.length > 0 && (
              <div className="flex flex-col gap-4 w-full">
                {historyType === 'all' && (
                  <h3 className="!p-3 text-[18px] font-black text-zinc-500 uppercase tracking-widest border-b border-[#ccc] pb-2">1.Lịch sử Tạo Ảnh AI</h3>
                )}
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full">
                  {images.map((item) => {
                    const displayRatio = item.aspectRatio ? (
                      item.aspectRatio === '9:16' ? '9:16 Dọc' :
                      item.aspectRatio === '16:9' ? '16:9 Ngang' : '1:1 Vuông'
                    ) : item.ratio;
                      ...
                      >
                        ...
                        <div className="flex flex-col gap-1 text-left flex-grow">
                          <div className="flex justify-between items-start">
                            <p className="text-xs font-bold text-[var(--text-primary)] truncate">{item.title || "Tạo Ảnh AI"}</p>
                        ...
                        <div className="flex gap-2 mt-2 pt-2 border-t border-[#222226]/60 w-full">
                          <button 
                            ...
                            className="flex-1 py-2 bg-[#18181c] hover:bg-zinc-800 border border-[#222226] hover:border-zinc-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 border-none transition-all cursor-pointer disabled:opacity-30 disabled:pointer-events-none"
                          >
                            <Download size={13} />
                            <span>Tải xuống</span>
                          </button>
